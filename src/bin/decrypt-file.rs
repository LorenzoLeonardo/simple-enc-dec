use enzo_crypto::{async_file_gcm, file_gcm::decrypt_file};
use std::{
    error::Error,
    io::{Write, stdout},
    path::PathBuf,
    time::Duration,
}; // assuming your crate name is enzo_crypto
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if progress {
        ...
        let dec_task = tokio::task::spawn({
            let plain = input_path.clone();
            let enc = output_path.clone();
            async move {
                async_file_gcm::decrypt_file(
                    &plain,
                    &enc,
                    &password,
                    tx_progress,
